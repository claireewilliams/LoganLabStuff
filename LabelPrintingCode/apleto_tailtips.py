...
def make_label(project, contact, sample_type, date, sample, replicate, num_replicates, label_width, label_height):
    ...
    font = ImageFont.truetype('Monaco.dfont', 30)
    contact_font = ImageFont.truetype('Monaco.dfont', 18)
    draw.text((10,10), string, (0,0,0), font=font)
    draw.text((10,85), contact_string, (0,0,0), font = contact_font)
    if (num_replicates > 1):
...
@click.option('--sample_type', '-t', required=False, type=str,
